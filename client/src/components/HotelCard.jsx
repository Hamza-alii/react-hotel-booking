...
const HotelCard = ({room, index}) => {
  return (
    ...
    <img src={room.images[0]} alt="" className='relative max-w-70 w-full rounded-xl overflow-hidden bg-white text-grey-500/90 shadoe-[0px_4px_4px_rgba(0,0,0,0.05)]'/>
   {index % 2 === 0 && <p className='px-3 py-1 absolute top-3 left-3 text-xs bg-white text-grey-800 font-medium rounded-full'>Best Seller</p>}
    <div className='p-4 pt-5'>
        ...
        <button className="px-4 py-2 text-sm font-medium border border-grey-300 rounded hover:bg-grey-50 transition-all cursor-pointer">
          Book Now
        </button>
        </div>
    </div>
